...
#En esta version se utilizan los simbolos de mayor o menor de "Preliminary Thermal Analysis of Small Satellites"
import numpy as np
...
#Defino las areas
A_total = 2*(0.1*0.1 + 0.1*0.3 + 0.1*0.3)
A_zen = 0.1*0.3
A_nad = 0.1*0.3
A_N = 0.1*0.3
A_S = 0.1*0.3
...
def view_factors(t, beta, f_E):
    if tau/4 > t % tau or t % tau > 3*tau/4:
    ...
    if tau/2*(1-f_E) > t%tau or t%tau > tau/2*(1+f_E):
        F_N = 0
        F_S = 0       
    else:
        if beta>0:
...
def six_nodes(t, T, beta, f_E, a, q_IR):
    ...
    Q_zen = F_zen*A_zen*q_solar*alpha + k*(A_i_zen_pv*(T_pv-T_zen) + A_i_zen_nv*(T_nv-T_zen) + A_i_zen_N*(T_N-T_zen) + A_i_zen_S*(T_S-T_zen)) - sigma*epsilon*A_zen*(T_zen**4) + Q_gen*A_zen/A_total
    ...
    Q_pv = F_pv*A_pv*q_solar*alpha + k*(A_i_pv_zen*(T_zen-T_pv) + A_i_pv_nad*(T_nad-T_pv) + A_i_pv_N*(T_N-T_pv) + A_i_pv_S*(T_S-T_pv)) - sigma*epsilon*A_pv*(T_pv**4) + Q_gen*A_pv/A_total

    Q_nv = F_nv*A_nv*q_solar*alpha + k*(A_i_nv_zen*(T_zen-T_nv) + A_i_nv_nad*(T_nad-T_nv) + A_i_nv_N*(T_N-T_nv) + A_i_nv_S*(T_S-T_nv)) - sigma*epsilon*A_nv*(T_nv**4) + Q_gen*A_nv/A_total

    Q_N = F_N*A_N*q_solar*alpha +  k*(A_i_N_pv*(T_pv-T_N) + A_i_N_nv*(T_nv-T_N) + A_i_N_zen*(T_zen-T_N) + A_i_N_nad*(T_nad-T_N)) - sigma*epsilon*A_N*(T_N**4) + Q_gen*A_N/A_total

    Q_S = F_S*A_S*q_solar*alpha + k*(A_i_S_pv*(T_pv-T_S) + A_i_S_nv*(T_nv-T_S) + A_i_S_zen*(T_zen-T_S) + A_i_S_nad*(T_nad-T_S)) - sigma*epsilon*A_S*(T_S**4) + Q_gen*A_S/A_total
