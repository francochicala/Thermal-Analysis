...
T_s = np.zeros((len(t),len(beta_vals))) #Es una matriz de ceros de tamaño: numero de segundos x numero de betas, cada columna es un beta fijo, Consultar a javi o saldia si es lo mejor
...
for j, beta_deg in enumerate(beta_vals): #para que me devuelva pares j y beta
    ...
    for i in range(len(t)-1): #es len(t) o len(t)-1????????
